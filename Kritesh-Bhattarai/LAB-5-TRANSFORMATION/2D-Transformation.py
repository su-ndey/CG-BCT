...
def plot_line_with_transformation(x0, y0, x1, y1):
    x_orig, y_orig = Bresen(x0, y0, x1, y1)
    ...
    composite_matrix = np.dot(np.dot(translation_matrix, scaling_matrix), rotation_matrix)
    ...
    plt.title("Bresenham Line with 2D Transformation")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    plt.grid(True)
